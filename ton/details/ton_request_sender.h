 ...
 public:
  ...
  class SpecificRequestBuilder final : public RequestBuilder {
   private:
    ...
    SpecificRequestBuilder(not_null<RequestSender *> sender, Request &&request) noexcept
   ...
   public:
    ...
    [[nodiscard]] SpecificRequestBuilder &fail(FnMut<void()> callback) noexcept {
    ...
    [[nodiscard]] SpecificRequestBuilder &done_async(FnMut<void()> callback) {
      setDoneHandler(std::move(callback));
      return *this;
    }
    [[nodiscard]] SpecificRequestBuilder &done_async(
        FnMut<void(const typename Request::ResponseType &result)> callback) {
      setDoneHandler([callback = std::move(callback)](LibResponse result) mutable {
        using FPointer = std::decay_t<decltype(tl_to(_request))>;
        using Function = typename FPointer::element_type;
        using RPointer = typename Function::ReturnType;
        using ReturnType = typename RPointer::element_type;
        callback(tl_from(tonlib_api::move_object_as<ReturnType>(std::move(result))));
      });
      return *this;
    }
    [[nodiscard]] SpecificRequestBuilder &fail_async(FnMut<void()> callback) noexcept {
      setFailHandler(std::move(callback));
      return *this;
    }
    [[nodiscard]] SpecificRequestBuilder &fail_async(FnMut<void(const TLError &error)> callback) noexcept {
      setFailHandler(std::move(callback));
      return *this;
    }

    RequestId send() {
      return RequestBuilder::send([copy = std::move(_request)] { return tl_to(copy); });
    }
  ...
  explicit RequestSender(Fn<void(const TLUpdate &)> updateCallback = nullptr);

  template <typename Request, typename = std::enable_if_t<!std::is_reference_v<Request> &&
...
RequestSender::SpecificRequestBuilder<Request> RequestSender::request(Request &&request) noexcept {
  return SpecificRequestBuilder<Request>(this, std::move(request));
}

template <typename Request, typename>
Result<typename Request::ResponseType> RequestSender::Execute(Request &&request) {
  auto response = Client::Execute(tl_to(request));
  Assert(response != nullptr);

  if (response->get_id() == tonlib_api::error::ID) {
    return ErrorFromLib(tl_from(tonlib_api::move_object_as<tonlib_api::error>(std::move(response))));
  }
  using FPointer = std::decay_t<decltype(tl_to(request))>;
  using Function = typename FPointer::element_type;
  using RPointer = typename Function::ReturnType;
  using ReturnType = typename RPointer::element_type;
  return tl_from(tonlib_api::move_object_as<ReturnType>(std::move(response)));
}
