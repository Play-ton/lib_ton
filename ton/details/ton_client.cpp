...
crl::time NextRequestResendDelay(crl::time currentDelay) {
  return !currentDelay ? kMinRequestResendDelay : std::clamp(currentDelay * 2, crl::time(1000), kMaxRequestResendDelay);
}

}  // namespace

Client::Client(Fn<void(LibUpdate)> updateCallback)
    : _updateCallback(std::move(updateCallback)), _thread([=] { check(); }) {
}
...
Client::~Client() {
  ...
  _handlers.clear();
  lock.unlock();
  send([] { return tonlib_api::make_object<tonlib_api::close>(); }, nullptr);
  _thread.join();
}

RequestId Client::send(Fn<LibRequest()> request, FnMut<bool(LibResponse)> handler) {
...
void Client::scheduleResendOnError(RequestId requestId) {
  crl::on_main(this, [=] {
    ...
    _resendTimer.call(nextDelay, [=] { resend(requestId); });
  });
}
  ...
  while (!_finished) {
    ...
    if (handler) {
      ...
      if (requestId) {
        ...
        _updateCallback(tonlib_api::move_object_as<tonlib_api::Update>(std::move(response.object)));
      }
    }
  }
}
