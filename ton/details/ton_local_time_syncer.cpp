...
LocalTimeSyncer::LocalTimeSyncer(BlockchainTime time, not_null<RequestSender *> lib, Fn<void()> destroy)
    : _lib(lib), _destroy(std::move(destroy)), _blockchainTime(time) {
  getLiteServerTime();
}
...
void LocalTimeSyncer::sync(const TLliteServer_Info &result) {
  result.match([&](const TLDliteServer_info &data) {
    ...
    if (IsTimeSimilarEnough(liteServerTime, blockchainTime) && !IsTimeSimilarEnough(liteServerTime, localTime)) {
...
bool LocalTimeSyncer::IsLocalTimeBad(BlockchainTime time) {
  return !IsTimeSimilarEnough(base::unixtime::now(), AdjustedBlockchainTime(time));
}
